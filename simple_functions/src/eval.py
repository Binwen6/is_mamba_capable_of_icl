...
def eval_batch(model, task_sampler, xs, xs_p=None):
    task = task_sampler()
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "lstm", "mamba", "s4"]:
        device = "cuda"
    else:
        device = "cpu"
...
def build_evals(conf, n_test_points=None):
    ...
    if task_name not in ["linear_regression"]:
...
if __name__ == "__main__":
    ...
    n_test_points = int(sys.argv[3]) if len(sys.argv) > 3 else 120
    for task in os.listdir(run_dir):
        task_dir = os.path.join(run_dir, task)
